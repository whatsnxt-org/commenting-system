...
const CommentType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    disLikedBy: { type: new GraphQLList(GraphQLID) },
  }),
});
...
const RootQuery = new GraphQLObjectType({
  ...
  fields: {
    comments: {
      ...
      async resolve(parent, { parentId, limit = 3, offset = 0 }) {
...
const Mutation = new GraphQLObjectType({
  ...
  fields: {
    ...
    createComment: {
      ...
      async resolve(parent, { content, author, parentId }) {
        const countDoc = await Comment.countDocuments({ parentId });

        if (parentId && countDoc >= config.commentDepth) {
          throw new Error(`Maximum nesting depth of ${config.commentDepth} exceeded`);
        }
    ...
    editComment: {
      ...
      async resolve(parent, { id, content }) {
        const comment = await Comment.findById(id);
        if (!comment) throw new Error('Comment not found.');
